...
> **주의:** WebhookFilter에 입력하는 시크릿 키는 반드시 `dku-toss-secret-2026`를 사용해야 합니다.
> 다른 키를 입력하면 DAST가 유효한 서명을 생성하지 못해 재전송 공격 탐지(WHSEC-004)가 실패합니다.
...
**전체 파이프라인 E2E 통합 테스트.** 프로젝트 루트에서 실행합니다.

```bash
python tests/test_integration.py
```

테스트 서버(`server/test_server.py`)를 포트 9200에서 자동 기동하고 아래 순서로 검증합니다.

1. **SAST 검증**: `vulnerable_webhook.py` 분석 → 규칙별 Finding 건수 확인, `secure_webhook.py` 분석 → 0건(오탐 없음) 확인
2. **DAST Probe**: 6개 엔드포인트 Tier 분류 확인
3. **DAST 공격**: 다운그레이드·재전송·타입 혼동 공격 실행 및 취약 판정 검증
4. **플랫폼별 검증**: Stripe, 토스페이먼츠, Slack, PortOne V2 엔드포인트 공격 검증
5. **등급 결정**: 통합 리포트 생성 → 전체 등급 F, SAST 7건+, DAST 4건+ 확인

모든 단계 통과 시 `🎉 전체 파이프라인 통과!` 출력 및 종료 코드 0을 반환합니다.
...
| CLI 분석 | `python main.py --code samples/shop_server.py --url http://localhost:8000 --secret dku-toss-secret-2024 --state-create /orders --state-query /orders/{id}` |
